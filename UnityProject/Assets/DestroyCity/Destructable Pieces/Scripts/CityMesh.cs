...
public class CityMesh : MonoBehaviour 
{
    ...
    public CityMeshTiles Tiles;

    public float scaleCoeff = 0.01f;
    ...
    public void Update()
    {
        UnityEngine.Profiling.Profiler.BeginSample("CityMesh.Update");
        foreach (var byTile in this.City.EnumerateChangeList().Select(qe => qe).GroupBy(pix => this.Tiles.GetTileAtCoords(pix)))
        {
            if (byTile.Key != null)
                byTile.Key.Paint(byTile, this.City);
        }
        this.City.FlushChangeList();
        UnityEngine.Profiling.Profiler.EndSample();
    }

    public void InitializeWithCityTexture(Texture2D texture)
    {

        this.City = new World(this.Origin, new Vector2I(MeshWidth, MeshHeight), this, texture);
        this.Tiles.InitializeWithCity(this.City.CityLayer);
    }
...
public class CityMeshTiles
{
    ...
    public CityMeshTiles(Vector2I origin, Vector2I worldSize, int tileSize, Material material)
    {
        ...
        //Debug.Log("tileHorizontal: " + tilesHorizontal);
        //Debug.Log("tileVertical: " + tilesVertical);

        // Create tiles of size Width * Height to fill the requested area
        this.TileArray = new CityMeshTile[tilesHorizontal, tilesVertical];
        for (int x = 0; x < tilesHorizontal; ++x)
            for (int y = 0; y < tilesVertical; ++y)
            {
                this.TileArray[x, y] = new CityMeshTile(new Vector2I(origin.x + x * tileSize, origin.y + y * tileSize), new Vector2I(tileSize, tileSize), material);
            }
    }

    // Apply this texture to the entire mesh, splitting between tile boundaries
    public void InitializeWithCity(Assets.Scripts.CityLayer CityLayer)
    {
        if (CityLayer.Width != this.Width || CityLayer.Height != this.Height)
            Debug.LogWarning(String.Format("World texture dimensions [{0}x{1}] do not match world mesh dimensions [{2}x{3}]", CityLayer.Width, CityLayer.Height, this.Width, this.Height));
        ...
        foreach (var tile in TileArray)
        {
            ...
            for (int i = 0; i < tilePixels.Length; ++i)
            {
                ...
                tilePixels[i] = CityLayer.GetAt(new Vector2I(x, y)); 
            }
            ...
            if (tile.Material == null)
                Debug.LogWarning("Did not set material to City, cannot apply City texture");
            else
                tile.Material.mainTexture = tileTexture;
        }
    }

    public CityMeshTile GetTileAtCoords(Vector2I coord)
    {
...
public class CityMeshTile
{
    ...
    public CityMeshTile(Vector2I from, Vector2I size, Material material)
    {
        this.FromX = from.x;
        this.FromY = from.y;
...
public class MeshComponents
{
    ...
    public MeshComponents(float _scaleCoeff)
    {
    ...
    public void GenerateSquareAt(float x, float y, float size)
    {
        ...
        Vertices.Add(new Vector3(x * scaleCoeff, (y + size) * scaleCoeff, 0));
        Vertices.Add(new Vector3((x + size) * scaleCoeff, (y + size) * scaleCoeff, 0));
        Vertices.Add(new Vector3((x + size) * scaleCoeff, (y) * scaleCoeff, 0));
        Vertices.Add(new Vector3(x * scaleCoeff, y * scaleCoeff, 0));
